...
public class VideoRecorder {
    ...
    public VideoRecorder(String filePath) {
        ...
        params = getDefaultEncodeParams();
    }

    public void setEncodeParams(EncoderParams params){
        this.params = params;
    }

    public EncoderParams getDefaultEncodeParams() {
        ...
        params.setAudioSampleRate(AACEncodeConsumer.DEFAULT_SAMPLE_RATE);  // 音频采样率通道数量
        params.setAudioFormat(AACEncodeConsumer.ENCODING_PCM_16BIT);       // 采
        params.setAudioChannelConfig(AACEncodeConsumer.CHANNEL_IN_MONO);// 单声道
        params.setAudioChannelCount(AACEncodeConsumer.CHANNEL_COUNT_MONO);       // 单声道样精度为16位
        return params;
    }
    ...
    public void start() {
        EncoderParams params = this.params;
        // 创建音视频编码线程
