...
LIBYUV_API
int UYVYToNV21(const uint8_t* src_uyvy, int src_stride_uyvy,
               uint8_t* dst_y, int dst_stride_y,
               uint8_t* dst_uv, int dst_stride_uv,
               int width, int height) {
  int y;
  int halfwidth = (width + 1) >> 1;
  void (*SplitVURow)(const uint8_t* src_uv, uint8_t* dst_u, uint8_t* dst_v,
                     int width) = SplitVURow_C;
  void (*InterpolateRow)(uint8_t* dst_ptr, const uint8_t* src_ptr,
                         ptrdiff_t src_stride, int dst_width,
                         int source_y_fraction) = InterpolateRow_C;
  if (!src_uyvy ||
...
#if defined(HAS_SPLITUVROW_SSE2)
  if (TestCpuFlag(kCpuHasSSE2)) {
    SplitVURow = SplitUVRow_Any_SSE2;
    if (IS_ALIGNED(width, 16)) {
      SplitVURow = SplitUVRow_SSE2;
    }
  }
#endif
#if defined(HAS_SPLITUVROW_AVX2)
  if (TestCpuFlag(kCpuHasAVX2)) {
    SplitVURow = SplitUVRow_Any_AVX2;
    if (IS_ALIGNED(width, 32)) {
      SplitVURow = SplitUVRow_AVX2;
    }
  }
#endif
#if defined(HAS_SPLITUVROW_NEON)
  if (TestCpuFlag(kCpuHasNEON)) {
    SplitVURow = SplitUVRow_Any_NEON;
    if (IS_ALIGNED(width, 16)) {
      SplitVURow = SplitVURow_NEON;
    }
  }
#endif
#if defined(HAS_INTERPOLATEROW_SSSE3)
...

  {
    ...
    for (y = 0; y < height - 1; y += 2) {
      // Split Y from UV.
      SplitVURow(src_uyvy, rows + awidth, rows, awidth);
      memcpy(dst_y, rows, width);
      SplitVURow(src_uyvy + src_stride_uyvy, rows + awidth * 2, rows, awidth);
      memcpy(dst_y + dst_stride_y, rows, width);
      InterpolateRow(dst_uv, rows + awidth, awidth, awidth, 128);
      src_uyvy += src_stride_uyvy * 2;
      dst_y += dst_stride_y * 2;
    ...
    if (height & 1) {
      // Split Y from UV.
      SplitVURow(src_uyvy, dst_uv, rows, awidth);
      memcpy(dst_y, rows, width);
    }
    free_aligned_buffer_64(rows);
  }
...
LIBYUV_API
int UYVYToNV12Mirror(const uint8_t* src_uyvy, int src_stride_uyvy,
               uint8_t* dst_y, int dst_stride_y,
               uint8_t* dst_uv, int dst_stride_uv,
               int width, int height) {
  int y;
...
// TODO(fbarchard): Mirror on mips handle unaligned memory.
...

  {
    ...
    // row of y and 2 rows of uv
    align_buffer_64(rows, awidth * 3);
    align_buffer_64(dst_uv_temp, awidth);

    for (y = 0; y < height - 1; y += 2) {
      // Split Y from UV.
      SplitVURow(src_uyvy, rows + awidth, rows, awidth);
      MirrorRow(rows, dst_y, width);
      //memcpy(dst_y, rows, width);
      SplitVURow(src_uyvy + src_stride_uyvy, rows + awidth * 2, rows, awidth);
      MirrorRow(rows, dst_y + dst_stride_y, width);
      //memcpy(dst_y + dst_stride_y, rows, width);
      InterpolateRow(dst_uv_temp, rows + awidth, awidth, awidth, 128); //3¨¦?¨´
      MirrorRow(dst_uv_temp, dst_uv, width); // ?¦Ì??
      src_uyvy += src_stride_uyvy * 2;
      dst_y += dst_stride_y * 2;
      dst_uv += dst_stride_uv;
    }
    ...
    free_aligned_buffer_64(rows);
    free_aligned_buffer_64(dst_uv_temp);
  }
  return 0;
}
// Mirror UYVYToNV21 with optional flipping
LIBYUV_API
int UYVYToNV21Mirror(const uint8_t* src_uyvy, int src_stride_uyvy,
               uint8_t* dst_y, int dst_stride_y,
               uint8_t* dst_uv, int dst_stride_uv,
               int width, int height) {
  int y;
